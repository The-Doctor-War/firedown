...
vAPI.Tabs = class {
    ...
    onUpdatedHandler(tabId, changeInfo, tab) {
        ...
        if ( changeInfo.status === 'complete' && pageStore) {
            const currentState = pageStore.getNetFilteringSwitch();
        ...
        //const curProfileBits = µBlock.blockingModeFromHostname(hn);
        //const noLargeMedia = sessionSwitches.evaluateZ('no-large-media', hn);
        //const noFonts = sessionSwitches.evaluateZ('no-remote-fonts', hn);
        //browser.runtime.sendNativeMessage("ublock", {firewall : {activated : currentState, profile: curProfileBits, media: noLargeMedia, fonts: noFonts}});
        //browser.runtime.sendNativeMessage("ublock", {firewall : {activated : currentState}});
...

{
    ...
    const manifest = browser.runtime.getManifest();
    const titleTemplate = manifest.browser_action
        ? `${manifest.browser_action.default_title} ({badge})`
        : 'uBlock Origin ({badge})';
    const icons = [
        { path: {
...
// Firedown: removed. The toolbar-icon dashboard/popup (popup-fenix.html) does
// not exist in this stripped build — the browser action is surfaced natively
// (ad-count badge via the host's ActionDelegate), so there is no dashboard to
// open on click. Extension tab/window creation is refused host-side in
// TabDelegate.onNewTab regardless of any remaining auxiliary-page opener.
...
vAPI.adminStorage = (( ) => {
    // Managed storage is not supported on Android (no native manifests),
    // so skip entirely to avoid logspam from NativeManifests.sys.mjs.
    if (
